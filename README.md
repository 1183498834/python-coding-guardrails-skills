...
| 6 | **测试验证** | 写完代码后**立即**从默认路径选取 **10 个样本**运行验证，**通过即完成（不先跑完整程序）**；未指定默认路径时**先询问使用者**，禁止编造路径或跳过测试 |
...
| `references/sampling-test.md` | 写完即测（10 样本冒烟测试、通过即完成不跑完整程序）、均匀抽样覆盖头中尾、运行断言、未指定路径时询问用户、失败修复后重跑全部 |
